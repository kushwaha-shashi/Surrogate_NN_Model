...
def main():
    ...
    pick_groups = all_groups[:100]  # group_id 0~99
    ...
    Nplots_dir = BASE_DIR / "plots"
    Nplots_dir.mkdir(exist_ok=True)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(Nplots_dir / "loss_curve.png", dpi=150)
    plt.close()
    ...
    if len(test_groups) > 0:
        ...
        plt.tight_layout()
        plt.savefig(Nplots_dir / f"scatter_ux_gid_{pick_gid}.png", dpi=150)
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(Nplots_dir / f"scatter_uy_gid_{pick_gid}.png", dpi=150)
        plt.close()

        #2D deformation plots (true vs predicted) for ux and uy
        ...
        plt.tight_layout()
        plt.savefig(Nplots_dir / f"deformation_true_gid_{pick_gid}.png", dpi=150)
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(Nplots_dir / f"deformation_pred_gid_{pick_gid}.png", dpi=150)
        plt.close()
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(Nplots_dir / f"deformation_overlay_gid_{pick_gid}.png", dpi=150)
        plt.close()

        print(f"Saved plots to: {Nplots_dir}")
